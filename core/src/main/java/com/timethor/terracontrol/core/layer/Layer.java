...
public abstract class Layer {
     ...
     * @return
     */
    public static Layer[] Init(long worldSeed, TerraWorld world) {
        ...
        TerraBiome[][] NormalBiomeMap = new TerraBiome[config.GenerationDepth + 1][];
        TerraBiome[][] IceBiomeMap = new TerraBiome[config.GenerationDepth + 1][];


        for (int i = 0; i < config.GenerationDepth + 1; i++) {
            ArrayList<TerraBiome> normalBiomes = new ArrayList<>();
            ArrayList<TerraBiome> iceBiomes = new ArrayList<>();
            ...
            if (!normalBiomes.isEmpty()) {
                NormalBiomeMap[i] = normalBiomes.toArray(new TerraBiome[normalBiomes.size() + config.normalBiomesRarity]);
            } else {
                NormalBiomeMap[i] = new TerraBiome[0];
            }

            if (!iceBiomes.isEmpty()) {
                IceBiomeMap[i] = iceBiomes.toArray(new TerraBiome[iceBiomes.size() + config.iceBiomesRarity]);
            } else {
                IceBiomeMap[i] = new TerraBiome[0];
            }
            ...
            for (BiomeConfig biomeConfig : config.biomeConfigs) {
                ...
                if (config.IsleBiomes.contains(biomeConfig.name) && biomeConfig.IsleInBiome != null) {
                    ...
                    for (String islandInName : biomeConfig.IsleInBiome) {
                        ...
                        if (islandIn == BiomeCatalog.OCEAN.Id) {
        ...
        if (config.biomeMode == TerraControl.getBiomeModeManager().FROM_IMAGE) {
        ...
        ZoomedLayer.propagateWorldSeed(worldSeed);
     ...
     * @param paramLong
     */
    public void propagateWorldSeed(long paramLong) {
        this.b = paramLong;
        if (this.child != null) {
            this.child.propagateWorldSeed(paramLong);
        }
