...
public class VeinGen extends Resource {
    ...
    public void load(List<String> args) throws InvalidConfigException {
        ...
        minAltitude = readInt(args.get(7), TerraControl.worldDepth, TerraControl.worldHeight - 1);
        maxAltitude = readInt(args.get(8), minAltitude + 1, TerraControl.worldHeight);
     ...
     *         starting vein.
     */
    public Vein getVeinStartInChunk(TerraWorld world, int chunkX, int chunkZ) {
        ...
        if (random.nextDouble() * 100.0 < veinRarity) {
            ...
            int veinY = RandomHelper.getRandomNumberInRange(random, minAltitude, maxAltitude);
            int veinZ = chunkZ * 16 + random.nextInt(16) + 8;
            int veinSize = RandomHelper.getRandomNumberInRange(random, minRadius, maxRadius);
