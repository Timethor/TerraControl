...
import com.timethor.terracontrol.core.custom.object.bo2.BO2Loader;
import com.timethor.terracontrol.core.custom.object.bo3.BO3Loader;
...
/**
 * This class is the registry for the custom object types. It also stores the global objects. World objects are stored in the WorldConfig class. <p /> <p/> Terrain Control supports multiple types of custom objects. By default, it supports BO2s, BO3s and a number of special "objects" like trees and UseWorld. <p /> <p/> All those implement CustomObject. Plugin developers can register their own custom object types. If you have a number of CustomObjects that you want to register, just add your object to the global objects in the onStart event using registerGlobalObject. If you have your own file format, just use registerCustomObjectLoader(extension, loader). <p /> <p/> Even trees are custom objects. If you want to add your own tree type, add your tree to the global objects and make sure that it's canSpawnAsObject returns false. <p /> <p/> If your object implements StructuredCustomObject instead of CustomObject, it will be able to have other objects attached to it, forming a structure. As long as each individual object fits in a chunk, Terrain Control will make sure that the structure gets spawned correctly, chunk for chunk.
 */
public final class CustomObjectManager {
    ...
    public void loadGlobalObjects() {
        ...
        TerraControl.getEngine().getGlobalObjectsDirectory().mkdirs();
        Map<String, CustomObject> loadedGlobalObjects = loadObjects(TerraControl.getEngine().getGlobalObjectsDirectory());
        TerraControl.log(Level.INFO, loadedGlobalObjects.size() + " global custom objects loaded.");
        this.globalObjects.putAll(loadedGlobalObjects);
    }

    /**
     * Registers a custom object loader. Register before the config files
     * are
     * getting loaded, please!
     *
     * @param extension The extension of the file. This loader will be
     *                  responsible for
    ...
    /**
     * Returns the CustomObject with the given name. It searches for a
     * world
     * object first, and then it searches for a global object.
     *
     * @param name  Name of the CustomObject, case-insensitive.
     * @param world The world to search in first before searching the
     *              global
     *              objects.
     * <p/>
     * @return The CustomObject, or null if there isn't one with that name.
     */
    public CustomObject getCustomObject(String name, TerraWorld world) {
     ...
     *
     * @param name   Name of the CustomObject, case-insensitive.
     * @param config The config to search in first before searching the
     *               global
     *               objects.
     * <p/>
     * @return The CustomObject, or null if there isn't one with that name.
    ...
    /**
     * Returns a Map with all CustomObjects in a directory and its
     * sub-directories
     ...
     *
     * @param directory The directory to load from.
     * @param enable    Whether or not to enable the CustomeObject's in
     * <p/>
     * @objects
     * @return
     */
    public Map<String, CustomObject> loadObjects(File directory, boolean enable) {
        ...
        if (enable) {
            TerraControl.log(Level.INFO, ("[BOB ENABLE INFO] {{BEGIN}} " + directory.getParentFile().getName().toUpperCase() + File.separator + directory.getName()).toUpperCase());
            for (CustomObject object : objects.values()) {
                object.onEnable(objects);
            }
            TerraControl.log(Level.INFO, ("[BOB ENABLE INFO] {{ END }} " + directory.getParentFile().getName().toUpperCase() + File.separator + directory.getName()).toUpperCase());
        }
        return objects;
    }

    /**
     * Parses a String in the format name(setting1=foo,setting2=bar) and
     * returns
     * a CustomObject.
     *
     * @param string
     ...
     */
    public CustomObject getObjectFromString(String string, TerraWorld world) {
