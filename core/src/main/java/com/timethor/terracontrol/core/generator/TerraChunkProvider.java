...
import com.timethor.terracontrol.core.custom.noise.NoiseGeneratorPerlinOctaves;
...
public class TerraChunkProvider {
    ...
    /**
     */
    private NoiseGeneratorPerlinOctaves octaveH;
    /**
     */
    private NoiseGeneratorPerlinOctaves octaveI;
    /**
     */
    private NoiseGeneratorPerlinOctaves octaveG;
    /**
     */
    private NoiseGeneratorPerlinOctaves octaveV;
    /**
     */
    private NoiseGeneratorPerlinOctaves octaveJ;
    /**
     */
    private NoiseGeneratorPerlinOctaves octaveK;
    /**
     */
    private double[] u;
    /**
     */
    private double[] octaveVResult = new double[256];
    /**
     */
    double[] octaveGResult;
    /**
     */
    double[] octaveHResult;
    /**
     */
    double[] octaveIResult;
    /**
     */
    double[] octageJResult;
    /**
     */
    double[] octaveKResult;
    /**
     */
     ...
     * @param world
     */
    public TerraChunkProvider(WorldConfig config, TerraWorld world) {
        ...
        this.octaveH = new NoiseGeneratorPerlinOctaves(this.rnd, 16);
        this.octaveI = new NoiseGeneratorPerlinOctaves(this.rnd, 16);
        this.octaveG = new NoiseGeneratorPerlinOctaves(this.rnd, 8);
        this.octaveV = new NoiseGeneratorPerlinOctaves(this.rnd, 4);

        this.octaveJ = new NoiseGeneratorPerlinOctaves(this.rnd, 10);
        this.octaveK = new NoiseGeneratorPerlinOctaves(this.rnd, 16);
    ...
    private void generateTerrain(int chunkX, int chunkZ, byte[] paramArrayOfByte) {
        ...
        int i6 = i1 + 1;
        if (this.worldSettings.biomeMode == TerraControl.getBiomeModeManager().OLD_GENERATOR) {
    ...
    boolean ReplaceForBiomeAndReturnWaterless(int paramInt1, int paramInt2, byte[] paramArrayOfByte) {
        ...
        double d1 = 0.03125D;
        this.octaveVResult = this.octaveV.Noise3D(this.octaveVResult, paramInt1 * 16, paramInt2 * 16, 0, 16, 16, 1, d1 * 2.0D, d1 * 2.0D, d1 * 2.0D);
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                int stone_noise = (int) (this.octaveVResult[(x + z * 16)] / 3.0D + 3.0D + this.rnd.nextDouble() * 0.25D);
                ...
                for (int y = this.heightMinusOne; y >= 0; y--) {
                    ...
                    if (y < 5 && (worldSettings.createAdminium(y)) && y <= this.rnd.nextInt(5)) {
                        ...
                        if (i10 == 0) {
                            i5 = -1;
                        } else if (i10 == MaterialCatalog.STONE.id) {
                            if (i5 == -1) {
                                if (stone_noise <= 0 && !this.worldSettings.removeSurfaceStone) {
                                    surfaceBlock = 0;
                                    groundBlock = (byte) MaterialCatalog.STONE.id;
                                ...
                                if ((i5 == 0) && (groundBlock == MaterialCatalog.SAND.id)) {
                                    i5 = this.rnd.nextInt(4);
                                    groundBlock = (byte) MaterialCatalog.SANDSTONE.id;
                                }
                            }
                        }
                    }
    ...
    private double[] GenerateTerrainNoise(double[] outArray, int paramInt1, int paramInt2, int paramInt3, int max_X, int max_Y, int max_Z) {
        ...
        if (this.worldSettings.oldTerrainGenerator) {
            this.octageJResult = this.octaveJ.Noise2D(this.octageJResult, paramInt1, paramInt3, max_X, max_Z, 1.121D, 1.121D);
        }
        this.octaveKResult = this.octaveK.Noise2D(this.octaveKResult, paramInt1, paramInt3, max_X, max_Z, 200.0D, 200.0D);

        this.octaveGResult = this.octaveG.Noise3D(this.octaveGResult, paramInt1, paramInt2, paramInt3, max_X, max_Y, max_Z, d1 / 80.0D, d2 / 160.0D, d1 / 80.0D);
        this.octaveHResult = this.octaveH.Noise3D(this.octaveHResult, paramInt1, paramInt2, paramInt3, max_X, max_Y, max_Z, d1, d2, d1);
        this.octaveIResult = this.octaveI.Noise3D(this.octaveIResult, paramInt1, paramInt2, paramInt3, max_X, max_Y, max_Z, d1, d2, d1);
        ...
        for (int x = 0; x < max_X; x++) {
            for (int z = 0; z < max_Z; z++) {
                ...
                double d3 = this.octaveKResult[i2D] / 8000.0D;
                if (d3 < 0.0D) {
                    d3 = -d3 * 0.3D;
                }
                d3 = d3 * 3.0D - 2.0D;
                ...
                for (int y = 0; y < max_Y; y++) {
                    ...
                    double d9 = this.octaveHResult[i3D] / 512.0D * biomeConfig.volatility1;
                    double d10 = this.octaveIResult[i3D] / 512.0D * biomeConfig.volatility2;

                    double d11 = (this.octaveGResult[i3D] / 10.0D + 1.0D) / 2.0D;
    ...
    private void oldTerrainNoise(int x, int z, int i4, int max_X, int max_Y, double d3) {
        ...
        this.VolatilityFactor = (this.octaveGResult[i4] + 256.0D) / 512.0D * this.VolatilityFactor;
