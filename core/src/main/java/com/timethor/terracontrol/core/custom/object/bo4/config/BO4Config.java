...
import com.timethor.terracontrol.core.configuration.ConfigFile;
...
import com.timethor.terracontrol.core.configuration.WorldConfig.ConfigMode;
import com.timethor.terracontrol.core.custom.object.CustomObject;
import com.timethor.terracontrol.core.custom.object.bo4.BO4;
import com.timethor.terracontrol.core.custom.object.bo4.checks.BO4Check;
import com.timethor.terracontrol.core.custom.object.bo4.BO4Settings;
...
import com.timethor.terracontrol.core.custom.object.bo4.BlockFunction;
import com.timethor.terracontrol.core.custom.object.bo4.BranchFunction;
import com.timethor.terracontrol.core.custom.object.bo4.config.SpawnConfig.Spawnables;
 ...
 * @author Timethor
 */
public class BO4Config extends ConfigFile {
    ...
    /**
     */
    public BO4Check[][] bo3Checks = new BO4Check[4][];
    /**
     */
     ...
     */
    public BO4Config(String name, File file, Map<String, CustomObject> otherObjectsInDirectory) {
        this.file = file;
        this.name = name;
    ...
    /**
     * Creates a BO3Config with the specified settings. Ignores the
     * settings in the
     * settings file.
     *
     * @param oldObject 
     * @param extraSettings 
     */
    public BO4Config(BO4 oldObject, Map<String, String> extraSettings) {
        ...
        this.settingsCache.put(TerraCatalog.SettingsMode.toString().toLowerCase(), ConfigMode.WriteDisable.toString());
    ...
    protected void readConfigSettings() {
        ...
        settingsMode = readSettings(TerraCatalog.SettingsMode);

        tree = readSettings(BO4Settings.tree);
        frequency = readSettings(BO4Settings.frequency);
        chance = readSettings(BO4Settings.rarity);
        rotateRandomly = readSettings(BO4Settings.rotateRandomly);
        spawnHeight = readSettings(BO4Settings.spawnHeight);
        minHeight = readSettings(BO4Settings.minHeight);
        maxHeight = readSettings(BO4Settings.maxHeight);
        maxBranchDepth = readSettings(BO4Settings.maxBranchDepth);
        excludedBiomes = readSettings(BO4Settings.excludedBiomes);

        sourceBlock = readSettings(BO4Settings.sourceBlock);
        maxPercentageOutsideSourceBlock = readSettings(BO4Settings.maxPercentageOutsideSourceBlock);
        outsideSourceBlock = readSettings(BO4Settings.outsideSourceBlock);
    ...
    private void readResources() {
        ...
        List<BO4Check> tempChecksList = new ArrayList<>();
        ...
        for (Map.Entry<String, String> entry : this.settingsCache.entrySet()) {
            ...
            if (start != -1 && end != -1) {
                ...
                ConfigFunction<BO4Config> res = TerraControl.getConfigFunctionsManager().getConfigFunction(functionName, this, this.name + " on line " + entry.getValue(), Arrays.asList(props));

                if (res != null && res.isValid()) {
                    if (res instanceof BlockFunction) {
                        tempBlocksList.add((BlockFunction) res);
                    } else if (res instanceof BO4Check) {
                        tempChecksList.add((BO4Check) res);
        ...
        bo3Checks[0] = tempChecksList.toArray(new BO4Check[tempChecksList.size()]);
    ...
    public void writeResources() throws IOException {
        ...
        for (BO4Check check : bo3Checks[0]) {
    ...
    protected void correctSettings() {
        ...
        chance = applyBounds(chance, 0.000001, 100.0);
        minHeight = applyBounds(minHeight, TerraControl.worldDepth, TerraControl.worldHeight - 1);
        maxHeight = applyBounds(maxHeight, minHeight, TerraControl.worldHeight);
        maxBranchDepth = applyBounds(maxBranchDepth, 1, Integer.MAX_VALUE);
        sourceBlock = applyBounds(sourceBlock, 0, TerraControl.supportedBlockIds);
    ...
    public void rotateBlocksAndChecks() {
        for (int i = 1; i < 4; i++) {
            ...
            // BO3 checks
            bo3Checks[i] = new BO4Check[bo3Checks[i - 1].length];
