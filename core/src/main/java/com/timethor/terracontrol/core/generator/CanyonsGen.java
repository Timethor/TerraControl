...
public class CanyonsGen extends TerrainGenBase {
     ...
     * @param world
     */
    public CanyonsGen(WorldConfig wrk, TerraWorld world) {
        super(world);
    ...
    protected void a(long paramLong, int chunk_x, int chunk_z, byte[] paramArrayOfByte, double paramDouble1, double paramDouble2, double paramDouble3, float paramFloat1, float paramFloat2, float paramFloat3, int size, double paramDouble4) {
        ...
        for (int stepCount = 0; stepCount < size; stepCount++) {
            ...
            int k = MathHelper.floor_int(paramDouble1 - d3) - chunk_x * 16 - 1;
            int m = MathHelper.floor_int(paramDouble1 + d3) - chunk_x * 16 + 1;

            int n = MathHelper.floor_int(paramDouble2 - d4) - 1;
            int i1 = MathHelper.floor_int(paramDouble2 + d4) + 1;

            int i2 = MathHelper.floor_int(paramDouble3 - d3) - chunk_z * 16 - 1;
            int i3 = MathHelper.floor_int(paramDouble3 + d3) - chunk_z * 16 + 1;
                ...
                for (int i6 = i2; (i4 == 0) && (i6 < i3); i6++) {
                    for (int i7 = i1 + 1; (i4 == 0) && (i7 >= n - 1); i7--) {
                        ...
                        if (i7 < worldSettings.WorldHeight) {
                            if ((paramArrayOfByte[i8] == MaterialCatalog.WATER.id) || (paramArrayOfByte[i8] == MaterialCatalog.STATIONARY_WATER.id)) {
                                i4 = 1;
                            }
                    ...
                    if (d9 * d9 + d10 * d10 < 1.0D) {
                        for (int i11 = i1 - 1; i11 >= n; i11--) {
                            ...
                            if ((d9 * d9 + d10 * d10) * this.a[i11] + d11 * d11 / 6.0D < 1.0D) {
                                ...
                                if (i12 == MaterialCatalog.GRASS.id) {
                                    i10 = 1;
                                }
                                if ((i12 == MaterialCatalog.STONE.id) || (i12 == MaterialCatalog.DIRT.id) || (i12 == MaterialCatalog.GRASS.id)) {
                                    if (i11 < 10) {
                                        paramArrayOfByte[i9] = (byte) MaterialCatalog.LAVA.id;
                                    } else {
                                        paramArrayOfByte[i9] = 0;
                                        if ((i10 != 0) && (paramArrayOfByte[(i9 - 1)] == MaterialCatalog.DIRT.id)) {
                                            paramArrayOfByte[(i9 - 1)] = (byte) MaterialCatalog.GRASS.id;
                                        }
                                    }
                                }
                            }
