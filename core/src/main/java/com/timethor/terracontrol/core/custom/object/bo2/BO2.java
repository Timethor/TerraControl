...
public final class BO2 extends ConfigFile implements CustomObject {
    ...
    public boolean spawnForced(TerraWorld world, Random random, Rotation rotation, int x, int y, int z) {
        ObjectCoordinate[] CoordData = this.data[rotation.getRotationId()];

        // Spawn
        for (ObjectCoordinate point : CoordData) {
    ...
    public boolean canSpawnAt(TerraWorld world, Rotation rotation, int x, int y, int z) {
        ...
        if (!spawnWater) {
            if ((checkBlock == MaterialCatalog.WATER.id) || (checkBlock == MaterialCatalog.STATIONARY_WATER.id)) {
                return false;
            }
        }
        if (!spawnLava) {
            if ((checkBlock == MaterialCatalog.LAVA.id) || (checkBlock == MaterialCatalog.STATIONARY_LAVA.id)) {
                return false;
            }
        }
        ...
        for (ObjectCoordinate point : coordData) {
            ...
            if (!dig) {
                if (collisionBlockType.contains(world.getTypeId((x + point.x), (y + point.y), (z + point.z)))) {
                    faultCounter++;
                    if (faultCounter > (coordData.length * (collisionPercentage / 100))) {
                        return false;
                    }
                }
            }
...
//            GenerateCustomObjectFromGroup(world, random, x, y, z);
    ...
    private HashSet<Integer> ReadBlockList(ArrayList<String> blocks, String settingName) {
        ...
        if (all || solid) {
            for (MaterialCatalog material : MaterialCatalog.values()) {
                if (material.id == 0) {
                    continue;
                }
