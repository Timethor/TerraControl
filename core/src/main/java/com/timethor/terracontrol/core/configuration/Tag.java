 ...
 * NBT IO class
 * <p/>
 * @see <a href="https://github.com/udoprog/c10t/blob/master/docs/NBT.txt">
 * Online NBT specification</a>
 */
public class Tag {
     ...
     *
     * @param name the name to look for. May be null to look for unnamed
     *             tags.
     * <p/>
    ...
    private void writePayload(DataOutputStream dos) throws IOException {
        switch (type) {
            ...
            case TAG_Compound:
                Tag[] subtags = (Tag[]) value;
                for (Tag st : subtags) {
                    Type tagType = st.getType();
                    dos.writeByte(tagType.ordinal());
    ...
    private void print(Tag t, int indent) {
        ...
        if (tagType == Type.TAG_Byte_Array) {
            byte[] b = (byte[]) t.getValue();
            System.out.println(": [" + b.length + " bytes]");
        } else if (tagType == Type.TAG_List) {
            ...
            indent(indent);
            System.out.println("}");
        } else if (tagType == Type.TAG_Compound) {
            Tag[] subtags = (Tag[]) t.getValue();
            ...
            indent(indent);
            System.out.println("}");
        } else if (tagType == Type.TAG_Int_Array) {
