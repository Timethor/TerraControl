 ...
 *
 */
public class TerraWorldChunkManager extends WorldChunkManager {

    private BiomeGenerator biomeManager;
    private ArrayList<BiomeGenBase> biomesToSpawnIn = new ArrayList<>();
    private ForgeWorld localWorld;

    public TerraWorldChunkManager(ForgeWorld world) {
        this.biomesToSpawnIn.add(BiomeGenBase.biomeList[BiomeCatalog.FOREST.Id]);
        this.biomesToSpawnIn.add(BiomeGenBase.biomeList[BiomeCatalog.PLAINS.Id]);
        this.biomesToSpawnIn.add(BiomeGenBase.biomeList[BiomeCatalog.TAIGA.Id]);
        this.biomesToSpawnIn.add(BiomeGenBase.biomeList[BiomeCatalog.DESERT_HILLS.Id]);
        this.biomesToSpawnIn.add(BiomeGenBase.biomeList[BiomeCatalog.FOREST_HILLS.Id]);
        this.biomesToSpawnIn.add(BiomeGenBase.biomeList[BiomeCatalog.JUNGLE.Id]);
        this.biomesToSpawnIn.add(BiomeGenBase.biomeList[BiomeCatalog.JUNGLE_HILLS.Id]);
    ...
    public ChunkPosition findBiomePosition(int paramInt1, int paramInt2, int paramInt3, List paramList, Random paramRandom) {
        ...
        for (int i3 = 0; i3 < arrayOfInt.length; i3++) {
            if (arrayOfInt[i3] >= BiomeCatalog.values().length) {
                continue;
            }
            int i4 = i + i3 % n << 2;
            int i5 = j + i3 / n << 2;
