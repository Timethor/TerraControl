...
import com.timethor.terracontrol.bukkit.commands.BaseCommand;
...
public class MapWriter implements Runnable {
    ...
    public MapWriter(TerraPlugin _plugin, World _world, int _size, Angle _angle, CommandSender _sender, int _offsetX, int _offsetZ, String _label) {
        this.plugin = _plugin;
        this.world = _world;
        this.size = _size;
        this.sender = _sender;
                ...
                for (BiomeConfig biomeConfig : bukkitWorld.getSettings().biomeConfigs) {
                    if (biomeConfig != null) {
                        try {
                            ...
                            TerraControl.log(Level.WARNING, "Wrong color in " + biomeConfig.Biome.getName());
