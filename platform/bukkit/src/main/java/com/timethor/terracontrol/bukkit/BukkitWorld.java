...
import com.timethor.terracontrol.bukkit.structuregens.VillageGen;
import com.timethor.terracontrol.bukkit.structuregens.StrongholdGen;
import com.timethor.terracontrol.bukkit.structuregens.MineshaftGen;
import com.timethor.terracontrol.bukkit.structuregens.RareBuildingGen;
import com.timethor.terracontrol.bukkit.structuregens.NetherFortressGen;
import com.timethor.terracontrol.core.*;
import com.timethor.terracontrol.core.biome.generator.BiomeGenerator;
import com.timethor.terracontrol.core.biome.generator.OldBiomeGenerator;
import com.timethor.terracontrol.core.builtin.BiomeCatalog;
import com.timethor.terracontrol.core.builtin.MaterialCatalog;
import com.timethor.terracontrol.bukkit.util.NBTHelper;
 ...
 * @author Timethor
 */
public class BukkitWorld implements TerraWorld {
    ...
    /**
     */
    private TerraChunkGenerator generator;
    /**
     */
    ...
    private static int nextBiomeId = BiomeCatalog.values().length;
    private static int maxBiomeCount = 256;
    private static TerraBiome[] biomes = new TerraBiome[maxBiomeCount];
    /**
     */
    private HashMap<String, TerraBiome> biomeNames = new HashMap<>();
    private static ArrayList<TerraBiome> defaultBiomes = new ArrayList<>();
    /**
     */
    ...
    static {
        for (int i = 0; i < BiomeCatalog.values().length; i++) {
    ...
    public BukkitWorld(String _name) {
        this.name = _name;
        for (TerraBiome biome : defaultBiomes) {
    ...
    public int getLiquidHeight(int x, int z) {
        ...
        for (int y = worldHeight - 1; y > 0; y--) {
            ...
            if (MaterialCatalog.getMaterial(id).isLiquid()) {
                return y;
            }
        }
        return -1;
    ...
    public int getSolidHeight(int x, int z) {
        ...
        TerraControl.log(Level.ALL, "Chunk.b(" + x + "," + z + ") = ");
        for (int y = getHighestBlockYAt(x, z) - 1; y > 0; y--) {
            int id = chunk.getTypeId(x & 0xF, y, z & 0xF);
            if (MaterialCatalog.getMaterial(id).isSolid()) {
                return y + 1;
            }
        }
        return -1;
    ...
    public void setBlock(final int x, final int y, final int z, final int typeId, final int data, final boolean updateLight, final boolean applyPhysics, final boolean notifyPlayers) {
        ...
        if (y < TerraControl.worldDepth || y >= TerraControl.worldHeight) {
            return;
        }
    ...
    public int getHighestBlockYAt(int x, int z) {
        ...
        x = x & 0xF;
        int y = chunk.b(x, z);
        TerraControl.log(Level.ALL, "Chunk.b(" + x + "," + z + ") = " + y);
        while (chunk.getTypeId(x, y, z) != MaterialCatalog.AIR.id && y <= worldHeight) {
    ...
    public MaterialCatalog getMaterial(int x, int y, int z) {
        ...
        return MaterialCatalog.getMaterial(id);
    }
    ...
    public TerraChunkGenerator getChunkGenerator() {
    ...
    public void enable(org.bukkit.World world) {
        ...
        if (mcWorld.worldProvider.getName().equals("Overworld")) {
            ...
            mcWorld.worldProvider = new TerraWorldProvider(this, this.world.worldProvider);
        }

        // Inject our own BiomeManager (called WorldChunkManager)
        Class<? extends BiomeGenerator> biomeModeClass = this.settings.biomeMode;
        if (biomeModeClass != TerraControl.getBiomeModeManager().VANILLA) {
            TerraWorldChunkManager worldChunkManager = new TerraWorldChunkManager(this);
            mcWorld.worldProvider.e = worldChunkManager;

            BiomeGenerator settableBiomeManager = TerraControl.getBiomeModeManager().create(biomeModeClass, this, new BiomeCacheWrapper(worldChunkManager));
            worldChunkManager.setBiomeManager(settableBiomeManager);
            setBiomeManager(settableBiomeManager);
        }
    ...
    public void disable() {
        ...
        if (world.worldProvider instanceof TerraWorldProvider) {
            world.worldProvider = ((TerraWorldProvider) world.worldProvider).getOldWorldProvider();
        }
    }

    public void setChunkGenerator(TerraChunkGenerator _generator) {
    ...
    public void attachMetadata(int x, int y, int z, Tag tag) {
        ...
        if (tileEntity != null) {
            ...
            TerraControl.log(Level.INFO, "Skipping tile entity with id " + nmsTag.getString("id") + ", cannot be placed at " + x + "," + y + "," + z + " on id " + world.getTypeId(x, y, z));
        }
    }
